...
class CommitteeService {
  ...
  async getMainCommittee(): Promise<CommitteeData | null> {
    try {
      ...
      const result = await db.query('SELECT * FROM committees WHERE name = $1 AND is_active = true', ['Main Committee']);
  ...
  async addCommitteeMember(committeeId: string, roleId: string, userId: string, termStart?: string, termEnd?: string): Promise<boolean> {
    try {
      ...
      await db.query(insertQuery, [committeeId, roleId, userId, termStart || null, termEnd || null]);
      return true;
    } catch (error) {
  ...
  async removeCommitteeMember(committeeId: string, userId: string): Promise<boolean> {
    try {
      ...
      await db.query(updateQuery, [committeeId, userId]);
      return true;
    } catch (error) {
  ...
  async getCommitteeRoles(committeeId: string) {
    try {
      ...
      const result = await db.query(rolesQuery, [committeeId]);
      return result.rows;
    } catch (error) {
