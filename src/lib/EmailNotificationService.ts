...
export class EmailNotificationService {
  ...
  /**
   * Send a discussion notification via email
   */
  static async sendDiscussionNotification(
    userId: string,
    ...
    try {
      ...
      const user = userResult.rows[0];
      const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000';
      const discussionUrl = `${baseUrl}/discussions/${discussionId}`;
      ...
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="padding: 20px; background-color: #f9f9f9; border-radius: 0 0 5px 5px;">
            <h1 style="color: #333; font-size: 24px;">Discussion Notification</h1>
            <p style="color: #555; font-size: 16px; line-height: 1.5;">
              Hello ${user.name},
            </p>
            <p style="color: #555; font-size: 16px; line-height: 1.5;">
      ...
      return await emailService.sendEmail({
        to: user.email,
        subject: `Discussion Update: ${discussionTitle}`,
        html
      }, 'discussion-notification', discussionId);
