...
export async function PUT(request: NextRequest, { params }: Props) {
  try {
    ...
    if (!isManager) {
      ...
      if (diffInHours > 3) {
        return NextResponse.json(
          { error: "You can only edit posts within 3 hours of creation" },
          { status: 403 }
        );
      }
    }
    ...
    const result = await Database.query(query, [title, content, tags || [], id]);
...
// DELETE /api/posts/[id] - Delete post (only by author within 3 hours or managers)
export async function DELETE(request: NextRequest, { params }: Props) {
  try {
    ...
    if (!isManager) {
      ...
      if (diffInHours > 3) {
        return NextResponse.json(
          { error: "You can only delete posts within 3 hours of creation" },
          { status: 403 }
        );
      }
    }
    ...
    await Database.query("DELETE FROM post_likes WHERE post_id = $1", [id]);
    await Database.query("DELETE FROM comments WHERE post_id = $1", [id]);
