...
echo "Comprehensive TwoFactorAuthService.ts fix..."
...
if [ -f "$file" ]; then
    ...
    # Fix all the db.query calls and template literals
    cat > "$file" << 'EOF'
...
export class TwoFactorAuthService {
  ...
  static async generateTotpSecret(email: string): Promise<string> {
...
EOF
    ...
    echo "✓ Fixed TwoFactorAuthService.ts with comprehensive rewrite"
else
